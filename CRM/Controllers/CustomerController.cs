...
namespace CRM.Controllers
{
    public class CustomerController : Controller
    {
        ...
        public ActionResult SendMessage(int id, string message)
        {
            ...
            using (BaseContext context = ContextFactory.SingleContextFactory.Get<BaseContext>())
            {
                User customer = context.Users.FirstOrDefault(c => c.Id == id);
                if (customer != null)
                {
                    userModel = Mapper.Map<User, UserViewModel>(customer);
                }
            }
            ...
            EmailService.SendEmail(new EmailViewModel(userModel.Email, "CRM Message!", message), userModel);
        ...
        public ActionResult Conversation(int id)
        {
            ...
            using (BaseContext context = ContextFactory.SingleContextFactory.Get<BaseContext>())
            {
                User customer = context.Users.FirstOrDefault(u => u.Id == id);
                if (customer != null)
                {
                    userModel = Mapper.Map<User, UserViewModel>(customer);
                }
            ...
            var mailings = EmailService.LoadMails(userModel);
        ...
        {
            UserViewModel customerModel = new UserViewModel();
            DateTime? lastReseivedDate = null;

            using (BaseContext context = ContextFactory.SingleContextFactory.Get<BaseContext>())
            {
                User user = context.Users.Include(m => m.Emails).FirstOrDefault(l => l.Id == id);
                if (user != null)
                {
                    customerModel = Mapper.Map<User, UserViewModel>(user);
                    lastReseivedDate = user.Emails.LastOrDefault()?.SentDate;
                }
            }

            var mailings = EmailService.GetMailings(lastReseivedDate, customerModel);
